...
function FindingsWorkspace({ actor, filter, findings, mutate }: { actor: SessionActor; filter: string; findings: Finding[]; mutate: Mutation }) {
  ...
  return <>
    ...
    <OperationTable title="Daftar temuan"><table><thead><tr><th>Temuan</th><th>Sumber</th><th>Severitas</th><th>Status</th><th>Tindak lanjut</th></tr></thead><tbody>{visible.map((item) => <tr key={item.finding_id}><td><strong>{item.title}</strong><small>{item.description}</small></td><td>{humanStatus(item.source_kind)}</td><td><span className={`alos-live-status ${item.severity.toLowerCase()}`}>{item.severity}</span></td><td>{humanStatus(item.status)}</td><td>{item.status === "OPEN" ? <button onClick={() => void mutate(() => apiRequest(`/api/v1/findings/${item.finding_id}/status`, { method: "PATCH", body: JSON.stringify({ status: "ACKNOWLEDGED" }) }), "Temuan telah diakui dan siap ditindaklanjuti.")} type="button">Akui</button> : item.status !== "RESOLVED" && item.status !== "DISMISSED" ? <button onClick={() => void mutate(() => apiRequest(`/api/v1/findings/${item.finding_id}/status`, { method: "PATCH", body: JSON.stringify({ status: "RESOLVED", resolution: "Resolved through the ALOS findings workspace." }) }), "Temuan ditandai selesai.")} type="button">Selesaikan</button> : <small>{item.resolution ?? "Ditutup"}</small>}</td></tr>)}</tbody></table>{visible.length === 0 ? <OperationEmpty text="Tidak ada temuan dalam scope atau filter ini." /> : null}</OperationTable>
  </>;
}
...
function ReportsWorkspace({ actor, definitions, filter, mutate, reports }: { actor: SessionActor; definitions: ReportDefinition[]; filter: string; mutate: Mutation; reports: Report[] }) {
  ...
  return <>
    ...
    <div className="alos-report-definitions">{visibleDefinitions.map((definition) => <article className="alos-operation-card" key={definition.report_definition_id}><div><span className="alos-live-status info">{definition.period}</span><small>{definition.schedule_expression ?? "On demand"}</small></div><strong>{definition.name}</strong><p>{humanStatus(definition.scope)} · {definition.review_required ? "Perlu review" : "Tanpa review"}</p><button onClick={() => void mutate(() => apiRequest(`/api/v1/report-definitions/${definition.report_definition_id}/generate`, { method: "POST", body: JSON.stringify({ idempotency_key: crypto.randomUUID() }) }), "Laporan DRAFT berhasil dihasilkan dengan provenance.")} type="button">Generate sekarang</button><ReportScheduleControl definition={definition} mutate={mutate} /></article>)}{visibleDefinitions.length === 0 ? <OperationEmpty text="Belum ada definisi laporan dalam scope ini." /> : null}</div>
    <OperationTable title="Laporan terbaru"><table><thead><tr><th>ID laporan</th><th>Status</th><th>Dibuat</th><th>Provenance</th></tr></thead><tbody>{visibleReports.map((item) => <tr key={item.report_id}><td><code>{item.report_id}</code></td><td>{humanStatus(item.status)}</td><td>{formatOperationalDate(item.created_at)}</td><td>{item.provenance.length} sumber</td></tr>)}</tbody></table>{visibleReports.length === 0 ? <OperationEmpty text="Belum ada laporan yang dihasilkan." /> : null}</OperationTable>
  </>;
}
