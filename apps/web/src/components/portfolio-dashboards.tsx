...
export function DivisionsOverviewDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await fetch("/api/v1/divisions/overview", {
          cache: "no-store",
          credentials: "same-origin",
          signal: controller.signal,
        });
        if (!response.ok) throw new Error("division overview request failed");
        setData((await response.json()) as DivisionsOverviewSnapshot);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!response.ok) throw new Error("project portfolio request failed");
        setData((await response.json()) as ProjectPortfolioSnapshot);
      } catch (error) {
        if ((error as Error).name !== "AbortError") setFailed(true);
      } finally {
