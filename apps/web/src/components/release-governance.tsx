...
import { normalizeGovernanceError, runtimeNextAction, type GovernanceUiError } from "@/lib/governance-errors";
...
export function ReleaseGovernance() {
  ...
  useEffect(() => {
    const viewTimer = window.setTimeout(() => {
      ...
      if (views.some((item) => item.key === requested)) setView(requested as ReleaseWorkspaceView);
    }, 0);
  ...
  const visibleRequests = useMemo(() => (data?.requests ?? []).filter((request) => {
    const matchesText = request.agent_key.toLowerCase().includes(requestSearch.trim().toLowerCase());
    return matchesText && (!requestState || request.state === requestState);
  }), [data?.requests, requestSearch, requestState]);

  function changeView(nextView: ReleaseWorkspaceView) {
    setView(nextView);
    window.history.replaceState(null, "", `/releases?view=${nextView}`);
  }
  ...
  async function registerTestCase() {
    ...
    try {
      ...
      if (actionError instanceof ApiError && actionError.status === 409) await refreshRequest(detail.change_request_id);
  ...
  function deleteTestCase(testKey: string) { if (!detail) return; setConfirmation({ title: "Delete Test Case?", impact: `${testKey} akan dihapus hanya jika belum mempunyai immutable Test Run Evidence.`, confirmLabel: "Delete Test Case", destructive: true, onConfirm: () => { setConfirmation(null); void performDeleteTestCase(testKey); } }); }
  async function performDeleteTestCase(testKey: string) { if (!detail) return; setWorking(true); setError(null); setNotice(""); try { await api(`/api/v1/release-requests/${detail.change_request_id}/test-cases/${encodeURIComponent(testKey)}`, { method: "DELETE" }); await refreshRequest(detail.change_request_id); setNotice(`Test case ${testKey} dihapus. Immutable evidence tidak terpengaruh.`); } catch (actionError: unknown) { handleError(actionError, setError, router); } finally { setWorking(false); } }
  ...
  async function logout() { try { await api<void>("/api/v1/auth/logout", { method: "POST" }); window.location.assign(new URL("/login", window.location.origin).href); } catch (logoutError: unknown) { setError(normalizeGovernanceError(logoutError)); } }
  ...
  return <main className="release-shell">
    ...
    <nav className="governance-subnav" aria-label="Area kerja release">{views.map((item) => <button aria-current={view === item.key ? "page" : undefined} className={view === item.key ? "active" : ""} key={item.key} onClick={() => changeView(item.key)} type="button">{item.label}</button>)}</nav>
    ...
    <section className="release-grid">
      <article className="panel release-list-panel"><div className="panel-heading"><div><p className="eyebrow">RELEASE REQUESTS</p><h2>{data.requests.length} Request</h2></div><span className="permission-readonly">Immutable history</span></div><div className="governance-filters"><input aria-label="Cari Agent release" onChange={(event) => setRequestSearch(event.target.value)} placeholder="Cari Agent…" value={requestSearch} /><select aria-label="Filter state release" onChange={(event) => setRequestState(event.target.value)} value={requestState}><option value="">Semua state</option>{["DRAFT", "TESTED", "IN_REVIEW", "APPROVED", "RELEASED", "ACTIVE", "SUSPENDED", "ROLLED_BACK", "RETURNED", "REJECTED"].map((state) => <option key={state}>{state}</option>)}</select></div><ol className="agent-list">{visibleRequests.map((request) => <li key={request.change_request_id}><button className={selectedRequestId === request.change_request_id ? "agent-row selected" : "agent-row"} onClick={() => void selectRequest(request.change_request_id)} type="button"><span className="agent-level">REL</span><span><strong>{request.agent_key}</strong><small>{request.semantic_version} · Maker tercatat</small></span><span className={`lifecycle-pill lifecycle-${request.state.toLowerCase()}`}>{request.state}</span></button></li>)}</ol>{data.requests.length === 0 ? <p className="empty-state">Belum ada release request. Buat Agent melalui GENESIS untuk memulai.</p> : visibleRequests.length === 0 ? <p className="empty-state">Tidak ada request yang sesuai filter.</p> : null}</article>
      <section className="release-content">
        {detail ? <article className="panel lifecycle-panel"><div className="panel-heading"><div><p className="eyebrow">CURRENT STATE</p><h2>{detail.agent_key} · {detail.semantic_version}</h2></div><span className={`lifecycle-pill lifecycle-${detail.state.toLowerCase()}`}>{detail.state}</span></div><div className="release-progress"><span style={{ width: `${progress?.percent ?? 0}%` }} /></div><p className="field-note">Tahap {progress?.current} dari {progress?.total} · lifecycle history tetap append-only.</p><dl className="status-guidance"><div><dt>CURRENT BLOCKER</dt><dd>{nextAction?.reason}</dd></div><div><dt>NEXT REQUIRED ACTION</dt><dd>{nextAction?.title}</dd></div><div><dt>NEXT ACTOR</dt><dd>{nextActor(detail.state)}</dd></div></dl><button className="next-action-card" onClick={() => nextAction && changeView(nextAction.view)} type="button"><strong>{nextAction?.title}</strong><span>{nextAction?.reason}</span><small>Buka area terkait →</small></button></article> : null}
        ...
        {view === "tests" ? detail ? <TestsView data={data} detail={detail} editingTestKey={editingTestKey} latestTestRuns={latestTestRuns} mayAmend={mayAmend} testForm={testForm} working={working} setEditingTestKey={setEditingTestKey} setTestForm={setTestForm} deleteTestCase={deleteTestCase} executeTest={executeTest} registerTestCase={registerTestCase} repairTestCase={repairTestCase} submitReview={submitReview} /> : <SelectRequest /> : null}
...
type TestsViewProps = { data: ReleaseData; detail: ReleaseRequestDetail; editingTestKey: string | null; latestTestRuns: Map<string, ReleaseRequestDetail["test_runs"][number]>; mayAmend: boolean; testForm: ReturnType<typeof defaultTestForm>; working: boolean; setEditingTestKey: (value: string | null) => void; setTestForm: React.Dispatch<React.SetStateAction<ReturnType<typeof defaultTestForm>>>; deleteTestCase: (key: string) => void; executeTest: (key: string) => void; registerTestCase: () => Promise<void>; repairTestCase: (test: ReleaseRequestDetail["test_cases"][number]) => void; submitReview: () => void };
function TestsView({ data, detail, editingTestKey, latestTestRuns, mayAmend, testForm, working, setEditingTestKey, setTestForm, deleteTestCase, executeTest, registerTestCase, repairTestCase, submitReview }: TestsViewProps) {
  const passedCategories = new Set(detail.test_cases.filter((testCase) => latestTestRuns.get(testCase.test_case_id)?.status === "PASSED").map((testCase) => testCase.category));
  const missing = releaseTestCategories.filter((category) => !passedCategories.has(category));
  const testKeyValid = /^[A-Z][A-Z0-9_]{2,79}$/.test(testForm.testKey.trim().toUpperCase());
  let fixtureError = "";
  try { const parsed: unknown = JSON.parse(testForm.fixture); if (!parsed || Array.isArray(parsed) || typeof parsed !== "object") fixtureError = "Fixture harus berupa objek JSON."; }
  catch { fixtureError = "JSON tidak valid. Periksa quote, koma, dan bracket."; }
  return <article className="panel test-registry-panel">
    <div className="panel-heading"><div><p className="eyebrow">TEST &amp; EVIDENCE</p><h2>Expected dibanding actual runtime</h2></div><span className="permission-readonly">Run evidence immutable</span></div>
    {mayAmend ? <><div className="builder-fields two-column-fields compact-fields">
      <label>Kategori<select disabled={Boolean(editingTestKey)} onChange={(event) => setTestForm(defaultTestForm(event.target.value as TestCategory, detail.agent_key))} value={testForm.category}>{releaseTestCategories.map((category) => <option key={category}>{category}</option>)}</select></label>
      <label>Test key<input aria-invalid={!testKeyValid} disabled={Boolean(editingTestKey)} onChange={(event) => setTestForm((form) => ({ ...form, testKey: event.target.value }))} value={testForm.testKey} />{!testKeyValid ? <span className="field-error">Gunakan huruf kapital, angka, dan underscore. Contoh: RELEASE_POSITIVE_FIXTURE</span> : null}</label>
      <label className="full-width">Fixture JSON<textarea aria-invalid={Boolean(fixtureError)} className="code-input" onChange={(event) => setTestForm((form) => ({ ...form, fixture: event.target.value }))} rows={7} value={testForm.fixture} />{fixtureError ? <span className="field-error">{fixtureError}</span> : null}</label>
      <label>Expected status<select onChange={(event) => setTestForm((form) => ({ ...form, expectedStatus: event.target.value }))} value={testForm.expectedStatus}><option>SUCCEEDED</option><option>FAILED</option><option>BLOCKED</option></select></label>
    </div><div className="builder-actions"><button disabled={working || !testKeyValid || Boolean(fixtureError)} onClick={() => void registerTestCase()} type="button">{working ? "Menyimpan…" : editingTestKey ? "Simpan perbaikan" : "Tambah test case"}</button>{editingTestKey ? <button className="secondary-button" disabled={working} onClick={() => setEditingTestKey(null)} type="button">Batal</button> : null}</div></> : <p className="field-note">Test case hanya dapat diubah oleh Maker saat DRAFT/RETURNED. Test Run Evidence bersifat read-only dan immutable.</p>}
    <div className="table-wrap evidence-table"><table><thead><tr><th>Test Key / Category</th><th>Expected</th><th>Actual / Result</th><th>Reason / Next Action</th><th>Evidence</th><th>Aksi</th></tr></thead><tbody>{detail.test_cases.map((testCase) => { const result = latestTestRuns.get(testCase.test_case_id); return <tr key={testCase.test_case_id}>
      <td><strong>{testCase.test_key}</strong><br /><small>{testCase.category}</small></td><td>{String(testCase.expected_assertions.status ?? "—")}</td>
      <td>{result ? <><span className={`test-result test-${result.status.toLowerCase()}`}>{result.status}</span><br /><small>Actual: {result.actual_status ?? "Tidak tersedia"}<br />Code: {result.error_code ?? "—"}</small></> : <span className="test-result">NOT RUN</span>}</td>
      <td>{result && result.status !== "PASSED" ? <small><strong>{result.block_reason ?? "Runtime tidak memenuhi expected status."}</strong><br />Next: {runtimeNextAction(result.error_code, result.block_reason)}</small> : result ? <small>Expected dan actual sesuai. Tidak ada tindakan.</small> : <small>Jalankan test sebagai Checker independen.</small>}</td>
      <td>{result ? <small>Run: {result.agent_run_id?.slice(0, 8) ?? "—"}<br />Ref: {result.correlation_id.slice(0, 8)}<br />{result.completed_at ? formatDateTime(result.completed_at) : "Belum selesai"}</small> : "—"}</td>
      <td>{mayAmend ? <button className="secondary-button" disabled={working} onClick={() => repairTestCase(testCase)} type="button">Edit case</button> : null}{mayAmend && !result ? <button className="danger-button" disabled={working} onClick={() => deleteTestCase(testCase.test_key)} type="button">Delete</button> : null}{canCheckRelease(data.actor.roles) && ["DRAFT", "RETURNED"].includes(detail.state) ? <button disabled={working} onClick={() => executeTest(testCase.test_key)} type="button">{working ? "Running…" : "Jalankan"}</button> : null}</td>
    </tr>; })}</tbody></table></div>
    {detail.test_cases.length === 0 ? <p className="empty-state">Belum ada test case. Maker harus menambahkan POSITIVE, NEGATIVE, REGRESSION, SECURITY, dan RECOVERY.</p> : null}
    {canCheckRelease(data.actor.roles) && ["DRAFT", "RETURNED"].includes(detail.state) ? <div className="builder-actions action-with-explanation"><button disabled={working || missing.length > 0} onClick={submitReview} type="button">{working ? "Mengirim…" : "Kirim evidence untuk review"}</button>{missing.length > 0 ? <small>Submit blocked: kategori belum PASSED — {missing.join(", ")}.</small> : <small>5/5 kategori PASSED. Evidence siap dikirim.</small>}</div> : <p className="safe-note">Next action: Checker independen menjalankan test dan mengirim evidence.</p>}
  </article>;
}
...
function ReviewsView({ data, detail, review, reviewGate, working, setReview, recordReview, requestControlAction }: { data: ReleaseData; detail: ReleaseRequestDetail; review: { decision: string; notes: string }; reviewGate: "BUSINESS" | "TECHNICAL" | null; working: boolean; setReview: React.Dispatch<React.SetStateAction<{ decision: string; notes: string }>>; recordReview: () => void; requestControlAction: ControlAction }) {
  const approvedGates = new Set(detail.reviews.filter((item) => item.decision === "APPROVED").map((item) => item.review_gate));
  const reviewBlocked = !approvedGates.has("BUSINESS") || !approvedGates.has("TECHNICAL");
  return <article className="panel review-panel"><div className="panel-heading"><div><p className="eyebrow">REVIEWS &amp; APPROVAL</p><h2>Gate manusia terpisah</h2></div><span className="permission-readonly">Append-only · No self-approval</span></div><div className="review-gates"><ReviewGateCard gate="BUSINESS" reviews={detail.reviews} /><ReviewGateCard gate="TECHNICAL" reviews={detail.reviews} /><div><span>DIRECTOR APPROVAL</span><strong>{detail.approver_user_id ? "APPROVER TERCATAT" : "PENDING"}</strong><small>Final approval tetap tindakan manusia berwenang.</small></div></div>{detail.state === "IN_REVIEW" && reviewGate ? <div className="builder-fields two-column-fields compact-fields"><label>Gate<input disabled value={reviewGate} /></label><label>Keputusan<select onChange={(event) => setReview((value) => ({ ...value, decision: event.target.value }))} value={review.decision}><option>APPROVED</option><option>RETURNED</option><option>REJECTED</option></select></label><label className="full-width">Catatan wajib<textarea aria-invalid={!review.notes.trim()} onChange={(event) => setReview((value) => ({ ...value, notes: event.target.value }))} rows={3} value={review.notes} />{!review.notes.trim() ? <span className="field-error">Catatan review wajib menjelaskan dasar keputusan.</span> : null}</label><div className="builder-actions full-width"><button disabled={working || !review.notes.trim()} onClick={recordReview} type="button">{working ? "Saving Review…" : `Catat review ${reviewGate}`}</button></div></div> : null}{!reviewGate && detail.state === "IN_REVIEW" ? <p className="safe-note">Next action: Business Reviewer sesuai scope divisi dan Technical Reviewer independen menyelesaikan review.</p> : null}<div className="builder-actions action-with-explanation">{detail.state === "IN_REVIEW" && canApproveRelease(data.actor.roles) ? <><button disabled={working || reviewBlocked} onClick={() => requestControlAction("approve", "Approve release", "Merekam persetujuan final setelah seluruh gate manusia lengkap.", "Request disetujui dan diaudit.")} type="button">{working ? "Approving…" : "Approve"}</button>{reviewBlocked ? <small>Approval blocked: Business Review dan Technical Review harus APPROVED.</small> : null}</> : null}{detail.state === "APPROVED" && canApproveRelease(data.actor.roles) ? <button disabled={working} onClick={() => requestControlAction("release", "Release ke staging", "Menandai versi siap di staging internal; Agent belum otomatis ACTIVE.", "Versi dirilis ke staging internal.")} type="button">{working ? "Releasing…" : "Release"}</button> : null}{detail.state === "RELEASED" && canApproveRelease(data.actor.roles) ? <><button disabled={working || detail.kill_switch_active} onClick={() => requestControlAction("activate", "Aktifkan Agent", "Agent mulai dapat dijalankan sesuai kontrak, permission, budget, dan policy.", "Agent berhasil diaktifkan.")} type="button">{working ? "Activating Agent…" : "Activate"}</button>{detail.kill_switch_active ? <small>Activation blocked: Kill Switch masih ACTIVE.</small> : null}</> : null}</div><div className="table-wrap"><table><thead><tr><th>Gate</th><th>Reviewer</th><th>Scope / Division</th><th>Keputusan</th><th>Catatan</th><th>Timestamp</th></tr></thead><tbody>{detail.reviews.map((item) => <tr key={`${item.review_gate}-${item.created_at}`}><td>{item.review_gate}</td><td>{item.reviewer_name ?? item.reviewer_user_id?.slice(0, 8) ?? "Reviewer"}</td><td>{item.division_code ?? (item.review_gate === "TECHNICAL" ? "IT / AI Governance" : "Workspace scope")}</td><td>{item.decision}</td><td>{item.notes}</td><td>{formatDateTime(item.created_at)}</td></tr>)}</tbody></table></div></article>;
}
...
function ReleaseSummary({ detail }: { detail: ReleaseRequestDetail }) { return <article className="panel"><div className="panel-heading"><div><p className="eyebrow">REQUEST SUMMARY</p><h2>Scope dan kontrol</h2></div><span className="permission-readonly">Audited</span></div><p className="safe-note">{detail.requirement}</p><dl className="review-list"><div><dt>Agent / version</dt><dd>{detail.agent_key} · {detail.semantic_version}</dd></div><div><dt>Requester / Sponsor</dt><dd>{detail.requested_by_user_id}</dd></div><div><dt>Release Maker</dt><dd>{detail.maker_user_id}</dd></div><div><dt>Checker</dt><dd>{detail.checker_user_id ?? "Belum ditetapkan"}</dd></div><div><dt>Approver</dt><dd>{detail.approver_user_id ?? "Belum tercatat"}</dd></div><div><dt>Kill switch</dt><dd>{detail.kill_switch_active ? "ACTIVE" : "Tidak aktif"}</dd></div></dl></article>; }
function ReviewGateCard({ gate, reviews }: { gate: "BUSINESS" | "TECHNICAL"; reviews: ReleaseRequestDetail["reviews"] }) { const result = reviews.find((item) => item.review_gate === gate); return <div><span>{gate} REVIEW</span><strong>{result?.decision ?? "PENDING"}</strong><small>{result ? `${result.reviewer_name ?? "Reviewer"} · ${result.division_code ?? "scope workspace"}` : "Menunggu reviewer independen."}</small></div>; }
...
function nextActor(state: ReleaseRequestDetail["state"]): string { const actors: Partial<Record<ReleaseRequestDetail["state"], string>> = { DRAFT: "Maker / QA Checker", RETURNED: "Maker / QA Checker", TESTED: "Business & Technical Reviewer", IN_REVIEW: "Business Reviewer / Technical Reviewer / Director", APPROVED: "Director (recorded approver)", RELEASED: "Director (recorded approver)", ACTIVE: "Operations / IT Lead", SUSPENDED: "Director / IT Lead", ROLLED_BACK: "Release Maker" }; return actors[state] ?? "Governance owner"; }
