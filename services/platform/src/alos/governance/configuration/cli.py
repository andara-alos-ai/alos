...
def main() -> None:
    ...
    approval_policy = PaymentApprovalPolicyRegistry(args.definitions_root).load()
    print(f"Valid: {len(registers)} canonical register; {mapping_count} mapping")
    print(
        "Valid: payment approval policy "
        f"{approval_policy.policy_id}@{approval_policy.version} ({approval_policy.status})"
    )
